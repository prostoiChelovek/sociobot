...
#define SOB_WDB_MAYBE_ARR_() SOB_WDB_CHECK(maybe_arr_(c))
...
enum wdb_res wdb_key(struct wdb_ctx * c, const char * v)
{
    if ((! c->got_key) || (! c->is_first_val)) {
        ...
        SOB_WDB_CHECK(add_literal_(c, v));
        SOB_WDB_CHECK(add_literal_(c, ": "));
...
enum wdb_res wdb_bool(struct wdb_ctx * c, int v)
{
    ...
    return add_literal_(c, v ? "true" : "false");
}
