...
***** "tg_msg_ids": list of telegram id as uint32. Valid only if "tg_msg_is_delivered" and not "tg_msg_is_deleted", long messages are split, max length - "tg_msg_ids_len"
...
*** Methods
**** Each method here is bound to "tg_mod". There may be multiple instances
**** "tg_init": start initialization of a "tg_mod"
- Second call with same "tg_mod" will do nothing unless "tg_stop" was called
...
***** Chat keys are in format "chat.<key>"
***** For each object there is a block of key-value pairs called a record
***** Records are separated with one or more empty lines (stray spaces and tabs ignored)
***** Different records can contain same keys
***** Keys inside one record are unique
***** Missing or extra keys produce an error 
***** Each chat has a key "id" that corresponds to "tg_chat" ("chat.id")
***** Message keys are in format "msg.<key>"
***** Each message has a key "id" that corresponds to "tg_msg" ("msg.id")
***** Button keys are in format "btn.<key>"
***** Each button has a key "id" that corresponds to "tg_btn" ("btn.id")
...
***** Lists are in format '<key> : "<first>", "<second>" , "<third>"'. Trailing "," is allowed
*** Example
#+begin_src C
sob_ev evs[228];
tg_mod tg;
...
*** Methods
**** Each method here is bound to "yk_mod". There may be multiple instances
**** "yk_init": start initialization of the module
- Events: "ev_yk_init", "ev_yk_init_fail"
- Second call with same "yk_mod" will do nothing unless "yk_stop" was called
...
**** Keys inside one record are unique
**** Missing or extra keys produce an error 
**** Each object has an "id" key that corresponds to "yk_payment"
*** Example
#+begin_src C
sob_ev evs[228];
yk_mod yk;
...
**** Keys inside one record are unique
**** Missing or extra keys produce an error 
**** Each record contains an "id" key that corresponds to "lesson"
...
***** "account_is_admin": true if admin, TODO not the right place but not worth its own module
***** "account_is_blocked_support": true if the user is forbidden to forward messages to the admin, TODO not the right place but not worth its own module
*** Events
...
**** "acc_block_support": forbids the user to send messages to the admin, TODO not the right place but not worth its own module
*** Data storage
**** File name: "$PWD/acc.txt"
**** Keys and values: ascii
**** Keys end with ":". Spaces and tabs around ":" are ignored
**** Values are enclosed in double quotes. Everything inside quotes is treated literally
**** Lists are in format 'key : "<first>", "<second>" , "<third>"'. Trailing "," are allowed
**** For each object there is a block of key-value pairs called a record
**** Records are separated with one or more empty lines (stray spaces and tabs ignored)
**** Different records can contain same keys
**** Keys inside one record are unique
**** Missing or extra keys produce an error 
**** Each object has a key "id" that corresponds to "account"
** "str": Strings storage
*** Data
